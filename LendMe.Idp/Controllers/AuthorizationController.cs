 ...
 namespace LendMe.Idp.Controllers
 {
     public class AuthorizationController : Controller
     {
         ...
         public async Task<IActionResult> Authorize()
         {
             ...
             if (HttpContext.Request.Method == "POST")
             {
                 ...
                 if (submit == "Accept")
                 {
                     ...
                     identity1.SetClaim(OpenIddictConstants.Claims.Subject, await _userManager.GetUserIdAsync(user))
                         ...
                         .SetClaim(OpenIddictConstants.Claims.UpdatedAt,
                             new DateTimeOffset(user.UpdatedAt).ToUnixTimeSeconds().ToString());
                     ...
                     identity1.SetResources(await _scopeManager.ListResourcesAsync(identity1.GetScopes())
                         .ToListAsync());
                     ...
                     return SignIn(new ClaimsPrincipal(identity1),
             ...
             if (request.HasScope(OpenIddictConstants.Permissions.Scopes.Phone))
             {
                 if (!string.IsNullOrEmpty(user.PhoneNumber))
                 {
                     ...
                     identity.SetClaim(OpenIddictConstants.Claims.PhoneNumberVerified,
                         user.PhoneNumberConfirmed.ToString().ToLower());
                 }
             }
         ...
         {
             // Добавьте логирование для проверки, что метод вызывается
             Console.WriteLine($"=== EXCHANGE METHOD CALLED ===");
